...
export default function BOMMasterPage() {
    ...
    // SupabaseクライアントをuseMemoでメモ化し、nullにならないことを保証
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const { data: pData } = await supabase.from('m_products').select('*').order('product_code');
            const { data: iData } = await supabase.from('m_items').select('*').order('item_code');
    ...
    const fetchBOM = async (productId: string) => {
        ...
        const { data } = await supabase
            .from('m_boms')
            .select(`*, m_items(item_name, item_code, unit)`)
    ...
    const handleSaveBOM = async () => {
        ...
        // 削除処理
        await supabase.from('m_boms').delete().eq('product_id', selectedProductId);

        const cleanEntries = bomEntries
            .filter(e => e.item_id && e.quantity > 0)
            .map(e => ({
        ...
        if (cleanEntries.length > 0) {
            const { error } = await supabase.from('m_boms').insert(cleanEntries);
            if (error) {
                alert('保存に失敗しました');
                return;
            }
