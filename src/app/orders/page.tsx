...
interface OrderDetail {
  id: string; // 安定したキー（crypto.randomUUID）
  product_name: string;
  product_code: string;
  flavor_type: string;
  quantity_cs: number;
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const init = async () => {
      // 修正: supabaseがnullの場合はエラーをセットして中断
      if (!supabase) {
        setFetchError('システム構成エラー: Supabase クライアントの初期化に失敗しました。');
        setLoading(false);
        return;
      }

      try {
        const [resD, resP, resB, resI] = await Promise.all([
          supabase.from('m_destinations').select('*'),
          supabase.from('m_products').select('*'),
          supabase.from('m_bom').select('*'),
          supabase.from('m_items').select('*'),
        ]);

        const firstError = [resD.error, resP.error, resB.error, resI.error].find(Boolean);
        if (firstError) {
          setFetchError(`マスタデータの取得に失敗しました: ${firstError.message}`);
          return;
        }
  ...
  const getFlavorsForProductName = useCallback(
    (name: string): FlavorOption[] =>
      productsMaster
        ...
        .map(p => ({ code: p.product_code, flavor: p.mfg_type })),
    [productsMaster]
  );
  ...
  const requiredItems = useMemo((): RequiredItem[] => {
    ...
    for (const detail of orderDetails) {
      ...
      for (const bom of bomMaster.filter(b => b.product_code === detail.product_code)) {
        const totalUsage = Number(bom.usage_rate) * detail.quantity_cs;
  ...
  const handleSaveOrder = useCallback(async () => {
    // 修正: supabaseの存在確認
    if (!supabase) {
      alert('システムの初期化に失敗しているため、保存できません。');
      return;
    }
    ...
    setIsSaving(true);
    try {
      const inserts = orderDetails.map(d => ({
        ...orderHeader,
        product_code: d.product_code,
        product_name_at_order: d.product_name,
        quantity_cs: d.quantity_cs,
        remarks: `味:${d.flavor_type} | ${orderHeader.remarks}`.trim(),
      }));

      const { error } = await supabase.from('t_orders').insert(inserts);
      ...
      console.error('[OrdersPage] save error:', err);
    } finally {
      setIsSaving(false);
    }
  }, [orderHeader, orderDetails, supabase]);
                  ...
                  <ul className="max-h-60 overflow-y-auto">
                    {filteredDestinations.length > 0 ? filteredDestinations.map(d => (
                      ...
                      >
                        ...
                        <span className="text-[10px] opacity-50 font-mono">{d.destination_code}</span>
                      </li>
                    )) : (
